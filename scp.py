...
class SCPClient(object):
    ...
    def _send_single_files(self, files):
        ...
        for name in files:
            ...
            chan = self.channel
            if self._progress:
                # send 0 progress to detect begin :)
                self._progress(name, size, 0)
            while file_pos < size:
    ...
    def _send_files(self, files, filter=[], recursive=False):
        ...
        @param files: list of files or paths to be transferred to remote
            destination (all files are put into the dest. dir!)
        @type files: either str or list
        @param filter: list of fnmatch filters (*,*.c,..) empty list = no
            filtering
        ...
        for base in files:
            if os.path.isfile(base):
                ...
                for root, fls in self._list_dir(base, filter,
                                                recursive=recursive):
                    # build file-list for transfer, and transfer file by file
                    pop, push = self._traverse_dir_instruction(last, root)
                    last = root
                    for i in range(pop):
                        self._send_popd()
                    for i in range(push):
                    ...
                    self._send_single_files([os.path.join(root, f)
                                            for f in fls])
            elif not os.path.isfile(base) and not os.path.isdir(base):
                # wildcarded filename
                for root, fls in self._list_dir(os.path.dirname(base),
                                                filters=os.path.basename(base),
                                                recursive=False):
                    self._send_single_files([os.path.join(root, f)
                                            for f in fls])
        ...
        for root in paths:
            for root, dirs, files in os.walk(root, topdown=True):
                if len(filters) > 0:
                    ffiles = []
                    for f in filters:
                        # join with prev. files
                        ffiles += fnmatch.filter(files, f)
    ...
    def _traverse_dir_instruction(self, a, b):
        '''
        ...
        if a[-1] == "/":
             # kill trailing / (to avoid extra pushes/pops)
            a = a[:-1]
        if b[-1] == "/":
            b = b[:-1]
        last = a.split("/")
        ...
        pos = 0
        for e in last:
            if pos >= len(current) or e != current[pos]:
                break
            pos += 1
